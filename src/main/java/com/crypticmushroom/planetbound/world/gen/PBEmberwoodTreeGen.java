...
public class PBEmberwoodTreeGen extends WorldGenTrees
{
    ...
    public PBEmberwoodTreeGen(boolean flag)
    {
        super(flag);
    }
    ...
    public boolean generate(World world, Random rand, BlockPos pos)
    {
        ...
        if (pos.getY() >= 1 && pos.getY() + height + 1 <= 256)
        {
            ...
            if (!allClear)
            {
                ...
                if ((blockUsing == PBBlocks.ronnian_grass || blockUsing == PBBlocks.ronnian_dirt || blockUsing == PBBlocks.ronnian_coarse_dirt) && pos.getY() < 256 - height - 1)
                {
                    ...
                    for (cz = pos.getY() - width + height; cz <= pos.getY() + height; ++cz)
                    {
                        ...
                        for (tx = pos.getX() - treeWidth; tx <= pos.getX() + treeWidth; ++tx)
                        {
                            ...
                            for (int tz = pos.getZ() - treeWidth; tz <= pos.getZ() + treeWidth; ++tz)
                            {
                                ...
                                {
                                    this.setBlockAndNotifyAdequately(world, tPos, PBBlocks.emberwood_leaves.getDefaultState());
                                }
                            }
                        }
                    }
